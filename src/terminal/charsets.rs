...
mod tests {
    ...
    fn test_from_designation() {
        ...
        assert_eq!(
            CharsetRef::from_designation('0'),
            Some(CharsetRef::DecSpecial)
        );
    ...
    fn test_known_charsets() {
        for code in KNOWN_CHARSETS {
            assert!(
                get_charset_map(code).is_some(),
                "Charset '{}' should be known",
                code
            );
        }
    }
